...
public class ByteCodeTask extends TaskInput {
  ...
  public void process() {
    ...
    Path generatedOutput = outputPath().resolve("generated");
    generatedOutput.toFile().mkdirs();
    if (!generatedOutput.toFile().exists()) {
      throw new RuntimeException(String.format("Unable to create generated output path %s", generatedOutput));
    }
    ...
    try {
      ...
      Main.Result result = Main.compile(
              TurbineOptions.builder()
                      ...
                      .setGensrcOutput(generatedOutput.toFile().toString())
                      .setResourceOutput(generatedOutput.toFile().toString())
