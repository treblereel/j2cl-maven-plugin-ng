...
class Long {
    ...
    toUnsignedString(opt_radix) {
        ...
        const longRadix = Long.fromInt(radix);
        // Divide as unsigned 64-bit numbers.
        let quotient = this.shiftRightUnsigned(1).div(longRadix).shiftLeft(1);
        let remainder = this.subtract(quotient.multiply(longRadix));
    ...
    negate() {
        const negLow = (~this.low_ + 1) | 0;
        const overflowFromLow = !negLow;
        const negHigh = (~this.high_ + overflowFromLow) | 0;
    ...
    static fromInt(value) {
        ...
        if (goog.DEBUG && value !== intValue) {
            throw new Error('value should be a 32-bit integer');
        }
    ...
    static isStringInRange(str, opt_radix) {
        ...
        const extremeValue = (str.charAt(0) == '-') ? MIN_VALUE_FOR_RADIX_[radix] :
 ...
 * @param {number} val The 32-bit integer in question.
 * @return {!Long} The corresponding Long value.
 * @nosideeffects
 * @noinline
 */
function getCachedIntValue(val) {
    return intCache[val] || (intCache[val] = new Long(val, val < 0 ? -1 : 0));
}
